 ...
 */
public class PrivateKeyParserUtil {
	...
	static {
		...
		parsers
			.add(new PemParser(PKCS1_RSA_HEADER, PKCS1_RSA_FOOTER, PrivateKeyParserUtil::createKeySpecForPkcs1Rsa, "RSA"));
		parsers.add(new PemParser(SEC1_EC_HEADER, SEC1_EC_FOOTER, PrivateKeyParserUtil::createKeySpecForSec1Ec, "EC"));
	...
	private static PKCS8EncodedKeySpec createKeySpecForSec1Ec(byte[] bytes) {
		...
		Assert.state(ecPrivateKey.isType(DerElement.ValueType.ENCODED, DerElement.TagType.SEQUENCE),
				"Key spec should be an ASN.1 encoded sequence");
		DerElement version = DerElement.of(ecPrivateKey.getContents());
		Assert.state(version != null && version.isType(DerElement.ValueType.PRIMITIVE, DerElement.TagType.INTEGER),
		...
		Assert.state(privateKey != null && privateKey.isType(DerElement.ValueType.PRIMITIVE, DerElement.TagType.OCTET_STRING),
	...
	private static int[] getEcParameters(DerElement parameters) {
		...
		Assert.state(contents.isType(DerElement.ValueType.PRIMITIVE, DerElement.TagType.OBJECT_IDENTIFIER),
	 ...
	 * @param content
	 */
	public static PrivateKey parseSmart(String content) {
	 ...
	 */
	public static PrivateKey parse(String resource) {
		try {
	...
	public static PrivateKey parseContent(String text) {
		try {
			for (PemParser pemParser : PEM_PARSERS) {
				PrivateKey privateKey = pemParser.parse(text);
				if (privateKey != null) {
			...
			throw new IllegalStateException("Error loading private key file for content ", ex);
		}
	}
